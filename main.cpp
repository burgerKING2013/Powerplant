...
Value getVar(string n) {
    if (!callStack.empty() && callStack.top().locals.count(n))
...
void run() {
    ...
    while (ip < code.size()) {
        ...
        switch (i.op) {
            ...
            case DEL: globals.erase(i.arg); break;
            case ADD: {
            ...
            case PRINT: {
                ...
                else if (v.type == 1) cout << v.str << endl;
                else if (v.type == 2) cout << "[array size " << v.arr.size() << "]" << endl;
                break;
            }
